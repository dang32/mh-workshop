#!/bin/bash
aws cloudformation list-exports --query 'Exports[?Name==`mh-workshop-security-'"$STAGE"'`].Value' --output text > rolearn.txt
aws cloudformation list-exports --query 'Exports[?Name==`mh-workshop-security-secrets-'"$STAGE"'`].Value' --output text > s3.txt
